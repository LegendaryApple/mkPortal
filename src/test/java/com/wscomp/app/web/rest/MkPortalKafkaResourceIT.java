...
import com.wscomp.app.config.KafkaProperties;
...
class MkPortalKafkaResourceIT {
    ...
    void setup() {
        ...
        MkPortalKafkaResource kafkaResource = new MkPortalKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/mk-portal-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/mk-portal-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
